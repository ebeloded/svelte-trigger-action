...
const createCallbacksTrigger = <T>() => {
	...
	return {
		...
		trigger(triggerParam?: T): Promise<any> {
			return Promise.all(callbacks.map((cb) => Promise.resolve(cb(triggerParam))));
		}
	};
};
...
export const createTriggerAction = <P = any, T = any | undefined>(
	...
	return {
		triggerAction: (node: HTMLElement, initialParam: P) => {
			...
			const cleanup = registerCallback((triggerParam?: T) => fn(node, param, triggerParam));
			return {
				update(param: P) {
					param = param;
				},
...
export const createAnimationTriggerAction = (globalClassName?: string) => {
	const { trigger, triggerAction } = createTriggerAction<string | undefined, string | undefined>(
	...
	return {
		...
		animationAction: (node: HTMLElement, className?: string) => triggerAction(node, className)
	};
};
